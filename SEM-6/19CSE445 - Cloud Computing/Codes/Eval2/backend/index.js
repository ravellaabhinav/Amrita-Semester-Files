...
const destBucketName = "sudhay-bucket"; //Mention your bucket name
let destFileName = "";
const storageClient = new Storage({
    keyFilename: "./gcpkey/mysamplecloudproject-339607-ccddfd7cf9c0.json", //Mention Your Key name
});
...
app.post("/submit", upload.single("housePhoto"), async (req, res) => {
    ...
    if (errors.length == 0) {
        try {
            const destFileName = `./uploadsHolding/${req.file.filename}`;
